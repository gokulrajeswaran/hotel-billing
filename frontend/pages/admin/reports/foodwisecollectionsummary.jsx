...
const FoodwiseCollectionSummary = () => {
    ...
    const fetchReport = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.get(`${API_URL}/api/reports/foodwise-collection`, {
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500 mt-1">View total quantity sold and collections grouped by food item</p>
                </div>
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-brand-primary text-white">
                            <tr>
                                ...
                                <th className="px-5 py-3 text-left text-[11px] font-black uppercase tracking-wider">Food Item</th>
                                <th className="px-5 py-3 text-left text-[11px] font-black uppercase tracking-wider">Category</th>
                                <th className="px-5 py-3 text-center text-[11px] font-black uppercase tracking-wider">Total Qty Sold</th>
                                ...
                                data.map((row, idx) => (
                                    <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                        ...
                                        <td className="px-5 py-3">
                                            <div className="font-medium text-gray-800">{row.nameEnglish}</div>
                                            {row.nameTamil && (
                                                <div className="text-xs text-gray-400">{row.nameTamil}</div>
                                            )}
                                        </td>
                                        <td className="px-5 py-3 text-gray-600">{row.category || '—'}</td>
                                        <td className="px-5 py-3 text-center text-gray-700">{row.totalQty}</td>
                        ...
                        {data.length > 0 && (
                            <tfoot className="bg-gray-100 border-t-2 border-brand-primary">
                                <tr>
                                    <td colSpan={3} className="px-5 py-3 text-[11px] font-black uppercase tracking-wider text-gray-700">Total</td>
                                    <td className="px-5 py-3 text-center font-black text-gray-800">{totalQty}</td>
...
export default FoodwiseCollectionSummary;
