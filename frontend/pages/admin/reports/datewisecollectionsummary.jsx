...
const DatewiseCollectionSummary = () => {
    ...
    const fetchReport = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.get(`${API_URL}/api/reports/datewise-collection`, {
    ...
    const totalBills = data.reduce((sum, row) => sum + (row.billCount || 0), 0);
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500 mt-1">View total collections grouped by date</p>
                </div>
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-brand-primary text-white">
                            <tr>
                                ...
                                <th className="px-5 py-3 text-center text-[11px] font-black uppercase tracking-wider">No. of Bills</th>
                                <th className="px-5 py-3 text-right text-[11px] font-black uppercase tracking-wider">Total Collection (₹)</th>
                            </tr>
                        </thead>
                        <tbody>
                            {data.length === 0 ? (
                                ...
                                data.map((row, idx) => (
                                    <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                        ...
                                        <td className="px-5 py-3 text-center text-gray-700">{row.billCount}</td>
                        ...
                        {data.length > 0 && (
                            <tfoot className="bg-gray-100 border-t-2 border-brand-primary">
                                <tr>
                                    <td colSpan={2} className="px-5 py-3 text-[11px] font-black uppercase tracking-wider text-gray-700">Total</td>
                                    <td className="px-5 py-3 text-center font-black text-gray-800">{totalBills}</td>
